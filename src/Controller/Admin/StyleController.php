...
class StyleController extends AbstractController
{
    ...
    #[Route('/admin/style/supression/{id}', name: 'admin_style_supression', methods :'GET')]
    public function supressionStyle(Style $style, EntityManagerInterface $manager)
    {
        $nbAlbums=$style->getAlbums()->count();
        if($nbAlbums>0){
            $this->addFlash("danger", "Malheureusement, vous ne pouvez pas supprimer ce style car $nbAlbums album(s) y sont associés");
        }else{
